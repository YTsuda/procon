...
if odds_max_v == evens_max_v then
  ...
  if evens.length == 0 then
    evens_second_max = [0,0]
  else
    evens_second_max = evens.max{|x,y| x[1] <=> y[1]}
  end

  if odds.length == 0 then
    odds_second_max = [0,0]
  else
    odds_second_max = odds.max{|x,y| x[1] <=> y[1]}
  end

  if evens_second_max[1] > odds_second_max[1] then
    evens_max_cnt = evens_second_max[1]
  else
    odds_max_cnt = odds_second_max[1]
  end
end
